        ...
        for (int i = 0; i <= d; i++) {
            ...
            for (int seedMatch : seedMatches) {
                ...
                if (startPosInGenome >= 0) {
                    ...
                    int mutations = 0;
                    for (int j = 0; j < n; j++) {
                        if ((startPosInGenome + j) < referenceGenome.length() && read.charAt(j) != referenceGenome.charAt(startPosInGenome + j)) {
                            mutations++;
                        }
                        if (mutations > d) {
                            break;
    ...
    private List<Integer> findSeedMatches(String seed) {
        ...
        while (low <= high) {
            ...
            if (cmp == 0) {
                ...
                int tempMid = mid;
                while (--tempMid >= 0 && seed.equals(referenceGenome.substring(suffixArray[tempMid], Math.min(suffixArray[tempMid] + seed.length(), referenceGenome.length())))) {
                    matches.add(suffixArray[tempMid]);
                }
                tempMid = mid;
                while (++tempMid < suffixArray.length && seed.equals(referenceGenome.substring(suffixArray[tempMid], Math.min(suffixArray[tempMid] + seed.length(), referenceGenome.length())))) {
                    matches.add(suffixArray[tempMid]);
                }
                break;
            } else if (cmp < 0) {
